...
function EventCard({ event }: { event: ProgressEvent }) {
  ...
  const label = t(`progress.agents.${event.agent}`, {
    defaultValue: event.agent,
  });
  const phaseLabel = t(`progress.phases.${event.phase}`, {
...
export default function ProgressPanel() {
    ...
    return (
      <Card size="small" title={t("progress.title")}>
        <Empty
          ...
          description={
            status === "idle"
              ? t("progress.waitUpload")
              : t("progress.waitStart")
          }
        />
      </Card>
    );
  ...
  const titleContent = (
    <Space size="small">
      {t("progress.titleCount", { count: progressEvents.length })}
      {status === "running" && (
        <Tag color="processing">{t("progress.running")}</Tag>
      )}
    </Space>
  );
